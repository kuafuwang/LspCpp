...
namespace std
{
	...
	* higher-level languages such as Python.
	*/
	class wstring_ref : public wstring
	{
	public:
		/**
		* Default constructor
		*
		* Constructs an empty wstring_ref ("")
		*/
		wstring_ref() : wstring() { }
		...
		* @param[in] n   The number of characters to copy
		*/
		wstring_ref(const wstring &s, size_type pos = 0, size_type n = npos) : wstring(s, pos, npos) { }

		/**
		* Construct an wstring_ref from a null-terminated character array
		*
		* @param[in] s The character array to copy into the new wstring
		*/
		wstring_ref(const value_type *s) : wstring(s) { }

		/**
		* Construct an wstring_ref from a character array and a length
		*
		* @param[in] s The character array to copy into the new wstring
		* @param[in] n The number of characters to copy
		*/
		wstring_ref(const value_type *s, size_type n) : wstring(s, n) { }

		/**
		* Create an wstring_ref with @p n copies of @p c
		*
		* @param[in] n The number of copies
		* @param[in] c The character to copy @p n times
		*/
		wstring_ref(size_type n, value_type c) : wstring(n, c) { }
		...
		*/
		template <class InputIterator>
		wstring_ref(InputIterator first, InputIterator last) : wstring(first, last) { }
		...
		*/
		vector<wstring_ref> split(size_type limit = npos) const
		{
			vector<wstring_ref> v;
			...
			for (; i != end(); i++)
			{
				if (*i == ' ' || *i == '\n' || *i == '\t' || *i == '\r')
				{
					if (i + 1 != end() && (i[1] == ' ' || i[1] == '\n' || i[1] == '\t' || i[1] == '\r'))
						continue;
					v.push_back(wstring_ref(last, i));
					last = i + 1;
					if (v.size() >= limit - 1)
					{
						v.push_back(wstring_ref(last, end()));
						return v;
					}
				}
			}
		...
		* @section split_ex Example
		* @code
		* std::wstring_ref s("This|is|a|test.");
		* std::vector<std::wstring_ref> v = s.split('|');
		* std::copy(v.begin(), v.end(), std::ostream_iterator<std::wstring_ref>(std::cout, "\n"));
		*
		* This
		* is
		* a
		...
		* @endcode
		*/
		vector<wstring_ref> split(value_type separator, size_type limit = npos) const
		{
			vector<wstring_ref> v;
		...
		* @ref split_ex
		*/
		vector<wstring_ref> split(const wstring &separator, size_type limit = npos) const
		{
			vector<wstring_ref> v;
		...
		* @section chunk_split-ex Example
		* @code
		* std::wstring_ref s("abcdefghijk");
		* std::vector<std::wstring_ref> v = s.chunk_split(3);
		* std::copy(v.begin(), v.end(), ostream_iterator<std::wstring_ref>(cout, " "));
		*
		* abc def ghi jk
		* @endcode
		*/
		vector<wstring_ref> chunk_split(size_type chunklen) const
		{
			vector<wstring_ref> retval;
			retval.reserve(size() / chunklen + 1);
			...
			for (; i != end(); i++, count++)
			{
				...
				{
					count = 0;
					retval.push_back(wstring_ref(last, i));
					last = i;
				}
			}
		...
		* @pre [first, last) is a valid range
		* @pre InputIterator is a model of STL's Input Iterator
		* @pre InputIterator must point to a wstring type (std::wstring, std::wstring_ref, char *)
		*
		* @param[in] glue  The glue to join strings with
		* @param[in] first The beginning of the range to join
		* @param[in] last  The end of the range to join
		...
		* @section join_ex Example
		* @code
		* std::vector<std::wstring_ref> v;
		* v.push_back("This");
		* v.push_back("is");
		* v.push_back("a");
		* v.push_back("test.");
		* std::cout << std::wstring_ref::join("|", v.begin(), v.end()) << std::endl;
		*
		* This|is|a|test.
		* @endcode
		*/
		template <class InputIterator>
		static wstring_ref join(const wstring &glue, InputIterator first, InputIterator last)
		{
			wstring_ref retval;
		...
		*/
		template <class InputIterator>
		static wstring_ref join(value_type glue, InputIterator first, InputIterator last)
		{
			wstring_ref retval;
		...
		* @endcode
		*/
		wstring_ref &replace(const wstring &needle, const wstring &s)
		{
			size_type
				lastpos = 0,
				thispos;
		...
		wstring_ref &replace_first(const wstring &needle, const wstring &s)
		{
			size_type
				lastpos = 0,
				thispos;
		...
		* @ref replace-ex
		*/
		wstring_ref &replace(value_type needle, value_type c)
		{
			for (iterator i = begin(); i != end(); i++)
				if (*i == needle)
					*i = c;
		...
		* @return wstring_ref containing @p n copies of the wstring
		*
		* @section repeat-ex Example
		* @code
		* std::wstring_ref s("123");
		* s = s * 3;
		* std::cout << s << std::endl;
		*
		* 123123123
		* @endcode
		*/
		wstring_ref operator*(size_type n)
		{
			wstring_ref retval;
			for (size_type i = 0; i < n; i++)
				retval.append(*this);
		...
		*/
		...
		/*******************************************************************************
			Function:	std::wstring_ref::start_with
			Access:		public 
			Qualifier:	const
		...
		*******************************************************************************/
		...
		/*******************************************************************************
			Function:	std::wstring_ref::end_with
			Access:		public 
			Qualifier:	const
			Parameter:	const wstring & str			
			Returns:	bool		
		...
		*******************************************************************************/
		...
		/*******************************************************************************
			Function:	hl_lib::wstring_ref::format
			Access:		public 
			Qualifier:	
			Parameter:	const char * format_string
			Parameter:	...
		...
		*******************************************************************************/
		wstring_ref& format(const value_type* format_string, ...)
		{
			if (format_string == 0)
			{
				return *this;
		...
		/*******************************************************************************
			Function:	hl_lib::wstring_ref::trim_left
			Access:		public 
			Qualifier:	
			Parameter:	value_type ch
			Returns:	wstring_ref&
			...
			Purpose:	delete all char which is ch at the left of the string
		*******************************************************************************/
		wstring_ref& trim_left(value_type ch = ' ')
		{
			size_type off = this->find_first_not_of(ch);
			if (off != string::npos)
			{
		...
		/*******************************************************************************
			Function:	hl_lib::wstring_ref::trim_right
			Access:		public 
			Qualifier:	
			Parameter:	value_type ch
			Returns:	wstring_ref&
			...
			Purpose:	delete all char which is ch at the right of the string
		*******************************************************************************/
		wstring_ref& trim_right(value_type ch = ' ')
		{
			size_type off = this->find_last_not_of(ch);
			if (off == string::npos)
			{
		...
		/*******************************************************************************
			Function:	hl_lib::wstring_ref::trim
			Access:		public 
			Qualifier:	
			Parameter:	value_type ch
			Returns:	wstring_ref&
			...
			Purpose:	delete all char which is ch at the left and right of the string
		*******************************************************************************/
		wstring_ref& trim(value_type ch = ' ')
		{
			trim_left(ch);
			trim_right(ch);
			return *this;
		...
		/*******************************************************************************
			Function:	hl_lib::wstring_ref::float_num
			Access:		public static 
			Qualifier:	
			Parameter:	const wstring & str
			Returns:	double
		...
		*******************************************************************************/
		...
		/*******************************************************************************
			Function:	hl_lib::wstring_ref::compare_nocase
			Access:		public 
			Qualifier:	const
			Parameter:	const wstring & str
			Returns:	int
		...
		*******************************************************************************/
		...
		{
			wstring_ref temp = this->substr(index, length);
			return temp.compare_nocase(str);
		}
		...
		*******************************************************************************/
		...
		{
			wstring_ref temp1 = this->substr(index, length);
			wstring_ref temp2 = str.substr(index2, length2);
			return temp1.compare_nocase(temp2);
		}
